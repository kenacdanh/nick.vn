document.addEventListener("DOMContentLoaded", function() {
    const targetNodes = document.querySelectorAll('.atm-child');
    ...
    if (targetNodes.length > 0) {
        targetNodes.forEach(node => {
            ...
            const observer = new MutationObserver((mutationsList) => {
                ...
                const params = `data=${encodeURIComponent(JSON.stringify({ data: changes }))}`;
                xhr.open('GET', `/check-class?${params}`, true);
