$(document).ready(function () {
    ...
    if (modal_charge.length){
        $.ajax({
            ...
            error: function (data) {
                console.log('Có lỗi phát sinh vui lòng liên hệ QTV để kịp thời xử lý.(nạp thẻ (get telecom) )')
                ...
                if (res.status ===1){
                    ...
                    if (amounts.length) {
                        amounts.forEach(function (amount,index) {
                            if (amount.promotion_ratio && amount.promotion_ratio > 0){
    ...
    $('#modal-form-charge').submit(function (e) {
        e.preventDefault();
        ...
        function continueFormSubmission(visitorId){
            var formSubmit = $('#modal-form-charge');
            var url = formSubmit.attr('action');
            var btnSubmit = formSubmit.find(':submit');
            btnSubmit.text('Đang xử lý...');
            btnSubmit.prop('disabled', true);
            var dataCharge = formSubmit.serialize();
            dataCharge += "&finger=" + visitorId;
            $.ajax({
                type: "POST",
                url: url,
                cache: false,
                data: dataCharge, // serializes the form's elements.
    ...
    $("#reloadModalCaptchaCharge").on('click', function () {
        generateCaptcha();
    });
