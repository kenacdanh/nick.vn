$(document).ready(function(){
    ...
    $('body').on('click','#btnPurchase',function(e){
        ...
        if (!auth_check){
            ...
            var html = '';
            html += '<button type="submit" class="btn c-theme-btn c-btn-square c-btn-uppercase c-btn-bold google_buy_service" id="d3" style="" >Xác nhận thanh toán<div class="row justify-content-center loading-data__buydichvu"></div></button>';
            html += '<button type="button" class="btn c-theme-btn c-btn-border-2x c-btn-square c-btn-bold c-btn-uppercase" data-dismiss="modal">Đóng</button>';
            $('.modal-footer__data').html('');
            $('.modal-footer__data').html(html);
        }
                    ...
                    htmlmodal += '<div class="modal-footer">';//l3
                        if (response.aucheck == 0){
                            ...
                            if (parseInt(response.balance) < parseInt(response.price)){
                                ...
                                htmlmodal += '<button type="submit" class="btn c-theme-btn c-btn-square c-btn-uppercase c-btn-bold google_buy_service" id="d3" style="position: relative" >';
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(response.status == 1){
                    ...
                    if(response.cash_limit_type ){
                        $('#resetCashLimit').modal('show');
                        $('#resetCashLimitNoti').html(response.message);
                        $('#resetCashLimit .cash_limit_type').val(response.cash_limit_type);
                        $('#resetCashLimit .price').val(response.price);
                        $('#resetCashLimit .module').val(response.module);
